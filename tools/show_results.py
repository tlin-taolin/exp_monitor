...
from tools.file_io import list_files
from tools.file_io import load_pickle, read_json
from tools.auxiliary import str2time
...
def load_raw_info_from_experiments(root_path, folder_name, parse_all):
    """load experiments.
    ...
    for folder_path in exp_folder_paths:
        try:
            element_of_info = _get_info_from_the_folder(
                folder_path, folder_name, parse_all
            )
...
def _get_info_from_the_folder(folder_path, folder_name, parse_all):
...
def reorganize_records(records, larger_is_better):
    # define attributes.
    attributes = ["time", "step", "loss", "top1", "accuracy", "ppl", "bits", "mse"]

    def _parse(name, lines, is_train=True):
        parsed_lines = collections.defaultdict(list)
        map_attributes = [
            "time",
            "local_index" if "averaged" not in name and is_train else "epoch",
            "loss",
            "top1",
            "accuracy",
            "ppl",
            "n_bits_to_transmit" if is_train else None,
            "mse",
        ]
    ...
    # all types of records: ['train', 'test-local_model'].
...
def summarize_info(
    records, arg_names, based_on="n_nodes", be_groupby="te_top1", larger_is_better=True
):
    # define header.
